...
\end{eqnarray}
%
is applied to the system of coupled equations in order to determine
the complex eigenvalues that result from the effect of the external dc
field. The Stark problem represents a higher challenge than the
field-free problem, in this case we are looking for a solution in the
complex plane and the determinant to be minimized is a complex number,
which means the condition to be zero should be applied to its norm.
...
simple asymptotic form. The \textsc{cap} results shown later on for
the Stark shifts and widths are obtained using $r_{c} = 12$. The
parameter $\eta_{c}$ indicates the strength of the \textsc{cap}. The
eigenvalues are computed for an equidistant mesh of strength
