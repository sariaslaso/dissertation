...
this work to focus on two of these methods, the concept of a complex
absorbing potential~(\textsc{cap}) introduced as a perturbation to the
molecular Hamiltonian and exterior complex scaling~(\textsc{esc}).

% add artificial potential introduced to absorb outgoing wave
% relevant parameters in a CAP (equation for the CAP used in the partial wave calculations)
% approach to obtain the resonances (to remove the perturbation introduced by the CAP)
A \textsc{cap} is an artificial imaginary potential that is added to
the Hamiltonian as a perturbation that performs an analytical
continuation of the Hamiltonian to the complex plane. As a
consequence, the diverging tale of the wave function is absorbed.
