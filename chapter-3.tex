...
so-called Latter correction~\cite{LatterCor_1955} is applied to ensure
that the effective potential converges asymptotically to $-1/r$, as
expected in a Coulomb potential:
%
\begin{eqnarray}
...
This section discusses the partial differential equation approach that
is implemented in order to calculate the relevant resonance parameters
in the problem of the H$_{2}$O molecule exposed to a strong dc
field. Having obtained an effective potential to defined the
field-free Schr\"{o}dinger equation~(\ref{eq:sch_noCS}) for an orbital
obtained in the \textsc{scf} method, the problem of the dc ionization
of the molecule is developed further.
