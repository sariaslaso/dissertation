...
sentitive to propagated errors, it is necessary in theses cases to
...
According to the Riss-Meyer approximation~\cite{RissMeyer_1993}, the
...
with $l_{\mathrm{max}}=3$ of the $3a_{1}$ \textsc{mo} is shown in
Figure~\ref{fig:3a1_RMtrajectory} for a field intensity of $F_{0} =
-0.1\ \mathrm{a.u.}$. The blue circles indicate the eigenvalues
corresponding to the interval $[0.003, \dots, 0.06]\ \mathrm{a.u.}$ of
the \textsc{cap} parameter in steps of $10^{-3}\ \mathrm{a.u.}$. The
stabilization point $(n = 0)$ is shown as a red cross, and the complex
energy values at the accumulation points for $n = 1, 2$ of the
...
respectively. The accumulation points for $n = 1, 2$ are in close
proximity to one another, but deviate from the stabilization point.
...
This section presents the numerical results obtained from implementing
a multipole decomposition of the model
potential~(\ref{eq:model_potential}) for the H$_{2}$O molecule
combined with the effect of a quadratic complex absorbing
potential. The effect of an external dc field is studied by means of
complex eigenvalues that result from the non-hermitian analysis of the
...
energies obtained originally with the model potential using a Gaussian
basis in a quantum chemistry program~\cite{illescas_2015} reveals that
the outermost orbital, $1b_{1}$, with its density perpendicular to the
molecular plane, converges rapidly, because the density has limited
overlap with the hydrogen atoms, as indicated in
Fig.~\ref{fig:h2o_1b1_1b2}. The values fall slightly below the quoted
values in Refs.~\cite{illescas_2015}. Comparison with the calculated
value for the most logical comparison point in terms of a local
self-consistent potential approach, namely the optimized potential
method~(\textsc{opm})~\cite{opm_2007} is also very good.
...
3a_{1}, 1b_{1}$, respectively.% cite photoelectron spectroscopy page
...
\begin{figure}
  ...
  \caption{Resonance parameters for the $1b_{1}$ \textsc{mo} of
    ...
    $l_{\mathrm{max}} = 2, 3$ are shown as green and red crosses,
