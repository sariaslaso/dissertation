...
origin. This method allows us to address the tunnelling and
...
\begin{figure}
  ...
  \caption{Scaling function corresponding to $\chi_{\mathrm{s}} =
    0.8\ \mathrm{rad}$, $r_{\mathrm{s}} = 13\ \mathrm{a.u.}$, and $1
...
isolated \textsc{mo} in a simple approximation. Recently, the problem
of small molecules in a dc field has revealed the effect of
...
tunnelling process, a modified \textsc{ecs} to the radial
...
\begin{eqnarray}
  ...
  \end{split}
  \label{eq:sch_tunnelling_params}
\end{eqnarray}
%
...
the Hamiltonian in Eq.~(\ref{eq:sch_tunnelling_params}) contains the
...
inside the grid, as the tunnelling barrier extends to larger $r$.
...
approximating the exact resonance energy is minimal.  This optimal
value depends on the order of the correction scheme. Assuming that the
next higher order corrected energy is a sufficiently good
approximation to the exact resonance energy, the condition to
determine the optimal $\tilde{\eta}$ can be expressed
as~\cite{RissMeyer_1993}
%
\begin{eqnarray}
  \begin{split}
...
\end{eqnarray}
%
The $n$th order Riss-Meyer correction can be interpreted as removal of
the artifact provided by the \textsc{cap} by $n$th order perturbation
theory.
...
characterize the tunnelling process of the quasi-stationary state when
...
is very close in the free-field limit. Since the tunnelling barrier is
...
tunnelling region H($1s$) has an ionization rate that is larger by
...
%based on other models for the \textsc{mo}s.
...
$1b_{1}$, indicates. In the tunnelling region, at $F_{0} =
...
tunnelling regime dominates. In this region, there is a marked increase
...
the given field range the tunnelling regime is dominant. As was
