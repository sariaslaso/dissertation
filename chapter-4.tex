...
treats the binding potential of the atom as a perturbation once the
electron has been promoted into the continuum where the external laser
field governs the electron dynamics, is the foundation to the
...
field, this phenomenon is known as multiphoton ionization
(\textsc{mpi}). The interaction with intense laser fields induces an
ac Stark shift of the atomic bound states that is responsible for the
peak suppression in the ionization spectrum as the field intensity
...
in the ionization potential of the atom, $I_{p}$, see
...
number of photons required for \textsc{mpi} to take place depends on
the typical binding energies of the initial states, $E_{i} =
...
and reach the continuum. While for the helium atom, $I_{p} \sim
...
rescattering induced plateau that is visible in measurements along a
broad energy spectrum~\cite{Paulus_1994plateau, Walker_1996}, and
emerged as a prevalent feature in the \textsc{ati} energy
...
\begin{figure}
  ...
  \caption{\textsc{ati} spectra for various noble gases, at a
    wavelength of $\lambda = 630\ \rm{nm}$ and an intensity of $I
...
generated spectra~\cite{Paulus_1994plateau}. In this section, we are
concerned with exploring the \textsc{ati} energy spectrum of such
electrons that interact further with the atomic core and rescatter.
...
studying the convergence of the \textsc{ati} matrix element that
generates the ionization spectrum. Our calculation considers a laser
...
\caption{\textsc{ati} spectrum of zero-range model for helium by a
