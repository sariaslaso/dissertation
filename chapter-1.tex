...
Alternative well-established methods to compute resonance states are
those of complex scaling~(\textsc{cs})~\cite{Simon_1979} and complex
absorbing potentials~(\textsc{cap}s)~\cite{RissMeyer_1993} in which
the Hamiltonian is extended analitically into the complex plane by an
artificial local potential formulated to absorb the diverging tale of
the resonance wave functions, and the resonance parameters are
obtained from the square-integrable eigenfunctions of the modified
non-Hermitian Hamiltonian without modifying the boundary conditions of
the problem. Extensions of these methods, such as the exterior complex
scaling~(\textsc{ecs})~[REFS], have been introduced in studies of
ionization of molecular structures and to investigate Stark resonances
induced by time-dependent fields~\cite{Krause_2014,ecsScrinzi}.
