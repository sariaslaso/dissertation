...
molecule~\cite{Moccia_JCP_2164,Moccia_1964}. The linear expansion
introduced in Roothaan's formalism, Eq.~(\ref{eq:lcao_roothaan}), can
be expressed in terms of Slater-type orbitals as follows
%
\begin{eqnarray}
  \begin{split}
...
\end{eqnarray}
%
where the expansion coefficients $c_{si}$ in are evaluated within the
same iterative process in order to minimize the electronic energy.
The $(n,l,m)$ values indicate the size of the basis set implemented
for a given molecule.
% mention the expansion of n,l,m values used by Moccia and their
% comments on improving the calculations
The \textsc{scf} calculations for the ground
state of H$_{2}$O reported in~\cite{Moccia_1964}
...
exponents $\zeta$, and an estimate of the expansion coefficients,
$c_{si}$, of the molecular orbitals are among these quantities. The
process of optimization of the nonlinear parameter $\zeta$ reveals a
strong correlation with the geometrical equilibrium configuration of
the molecule. The numerical results of the one-centre basis
\textsc{scf} method
