...
\end{eqnarray}
%
where $H$ is the Hamiltonian operator that represents the field of the
nuclei alone, $J_{j}$ is the Coulomb operator, and $K_{j}$ is the
exchange operator.
\textcolor{blue}{[DEFINE THE OPERATORS HERE, EQS 2.8a AND 2.8b]}
...
\textsc{hf} \textsc{scf} problem can be addressed as the problem of
...
\textsc{hf} \textsc{scf} wave function provided the basis
